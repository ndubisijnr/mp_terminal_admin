  ...
  <div class="flex justify-center h-screen items-center">
    ...
    <div v-if="passwordResetStage === '1'" class="w-3/12 min-w-fit">
      ...
      <form class="" @submit.prevent="initiateForgotPassword">
        <base-input type="email" v-model="AuthRequest.InitiatePasswordResetRequest.userEmail"
    ...
    <div v-else class="w-3/12 min-w-fit">
      ...
      <p class="text-[14px] leading-[25px] text-center text-gray-500">OTP was sent to
        <code><span class="text-primary">{{ AuthRequest.InitiatePasswordResetRequest.userEmail }}</span></code>
      </p>
      <code>
              <p class="text-[14px] leading-[25px] text-center text-primary">{{ AuthRequest?.InitiateEnrollmentRequest?.userEmail }}</p>
          </code>
      <form class="" @submit.prevent="completeForgotPassword">
        <div>
          <base-input type="text" v-model="AuthRequest.CompletePasswordResetRequest.otp" placeholder="Enter OTP"
            label="Enter OTP" />
          <div class="w-100 flex justify-end">We can resend OTP in: 11:00</div>
        </div>
        <base-input type="password" v-model="AuthRequest.CompletePasswordResetRequest.userPassword"
